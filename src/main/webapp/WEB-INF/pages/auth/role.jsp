...
<body>
	...
	<div id="body" class="body page_user">
						...
						<div class="global_mod  float_layout query_area">
							<div class="global_info">
								<div class="search_bar query_field" id="searchBar">
									...
									</a> <input type="text" id="searchrolename" name="searchrolename"
										value="" class="frm_input jsSearchInput" placeholder="请输入用户名称">
									</span>
								</div>
									...
									<div style="overflow-x: scroll; padding-bottom: 20px;">
										<table class="table" cellspacing="0" style="width: 100%;">
											...
											<tbody class="tbody" id="userGroups">
												<c:forEach items="${roleInfoList}" var="roleInfoList"
													varStatus="status">
													<tr>
														<td class="table_cell">${status.index +1}</td>
														<td class="table_cell">${roleInfoList.rolename}</td>
														<td class="table_cell">${roleInfoList.createtime}</td>
														<c:if test="${roleInfoList.status==0}">
															<td class="table_cell">已启用</td>
														</c:if>
														<c:if test="${roleInfoList.status==1}">
															<td class="table_cell">已禁用</td>
														</c:if>
														<!--<td class="table_cell user_group_opr"><a
														href="javascript:;" class="mod_link js_tag_edit_btn">编辑</a><a
														class="mod_link " href="javascript:;">删除</a></td>-->
													</tr>
		...
		<div
			...
			<div class="dialog">
				<div class="dialog_hd">
					<h3>
						<span id="dialog_action">新增</span>角色
					</h3>
										...
										<li class="account_setting_item wrp_pic_item_spe2">
											<div class="center" id="searchBar">
												<label class="frm_input_box label">角色名称</label> <span
													...
													placeholder="请输入角色名称（支持中文）" required>
												</span>
											</div>
										</li>
									</ul>
				...
				<div class="dialog_ft">
					...
					<span class="btn btn_primary btn_input js_btn_p"><button
							type="button" class="js_btn" data-index="0">确定</button></span> <span
	...
	<script type="text/javascript">
		...
		function search() {
			...
			temp.action = "../rolenfo/showRoleInfo.do";
			temp.method = "post";
